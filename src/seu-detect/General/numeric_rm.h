...
/*
    ...
    相对误差约等于
*/
template <typename InputValType1, typename InputValType2,
          typename ToleranceType>
inline bool relApproxEqual(const InputValType1 val1, const InputValType2 val2,
                           const ToleranceType relTolerance) {
	return (abs(val1 - val2) < val2 * relTolerance ||
	        abs(val1 - val2) < val1 * relTolerance);
}
...
template <typename InputValType1, typename InputValType2,
          typename ToleranceType>
inline bool absApproxEqual(const InputValType1 val1, const InputValType2 val2,
                           const ToleranceType absTolerance) {
...
template <typename InputValType1, typename InputValType2,
          typename ToleranceType>
inline bool absApproxEqual(const InputValType1 val1, const InputValType2 val2,
                           const ToleranceType absToleranceMin,
                           const ToleranceType absToleranceMax) {
	return (abs(val1 - val2) < absToleranceMax &&
	        abs(val1 - val2) > absToleranceMin);
}
/*
