...
cv::RotatedRect operator+(const cv::RotatedRect &rec, const cv::Point_<T> &pt) {
	return cv::RotatedRect(
	    cv::Point2f(rec.center.x + pt.x, rec.center.y + pt.y), rec.size,
	    rec.angle);
}
} // namespace cv
...
const cv::Point2f
crossPointOf(const std::array<cv::Point_<ValType>, 2> &line1,
             const std::array<cv::Point_<ValType>, 2> &line2) {
	...
	if (d == 0.0) {
		...
		return cv::Point2f(float(b1 * c2 - b2 * c1) / d,
		                   float(c1 * a2 - c2 * a1) / d);
	}
}
inline const cv::Point2f crossPointOf(const cv::Vec4f &line1,
                                      const cv::Vec4f &line2) {
	const std::array<cv::Point2f, 2> line1_{
	    cv::Point2f(line1[2], line1[3]),
	    cv::Point2f(line1[2] + line1[0], line1[3] + line1[1])};
	const std::array<cv::Point2f, 2> line2_{
	    cv::Point2f(line2[2], line2[3]),
	    cv::Point2f(line2[2] + line2[0], line2[3] + line2[1])};
	return crossPointOf(line1_, line2_);
}

/*template<typename ValType>
const cv::Point2f crossPointOf(std::initializer_list<ValType> line1,
std::initializer_list<ValType> line2)
{
    ValType a1 = line1[0].y - line1[1].y;
    ValType b1 = line1[1].x - line1[0].x;
    ValType c1 = line1[0].x*line1[1].y - line1[1].x*line1[0].y;
 ...
 */
template <typename T1, typename T2>
const cv::Rect_<T1> scaleRect(const cv::Rect_<T1> &rect,
                              const cv::Vec<T2, 2> scale,
                              cv::Point_<T1> anchor = cv::Point_<T1>(-1, -1)) {
	if (anchor == cv::Point_<T1>(-1, -1)) {
		anchor =
		    cv::Point_<T1>(rect.x + rect.width / 2, rect.y + rect.height / 2);
	}
	cv::Point_<T1> tl(
	    (cv::Vec<T1, 2>(rect.tl()) - cv::Vec<T1, 2>(anchor)).mul(scale) +
	    cv::Vec<T1, 2>(anchor));
...
void showContour(const std::string &windowName, const cv::Mat &srcImg,
                 cv::Mat &dstImg, ContourType contour, const cv::Scalar &color,
	...
	cv::drawContours(dstImg, std::vector<ContourType>{contour}, -1, color, 1, 8,
	                 cv::noArray(), INT_MAX, offset);
...
void showContours(const std::string &windowName, const cv::Mat &srcImg,
                  cv::Mat &dstImg, const ContoursType &contours,
                  const cv::Scalar &color, const int waitTime = -1,
                  const cv::Point offset = cv::Point(0, 0)) {
	if (srcImg.channels() == 1)
		cvtColor(srcImg, dstImg, cv::COLOR_GRAY2BGR);
	else if (srcImg.data != dstImg.data)
...
template <typename RecType> struct showRectangle_wrapper<RecType, true> {
	static void showRectangle(const std::string &windowName,
	                          const cv::Mat &srcImg, const cv::Mat &dstImg,
	                          RecType &rec, const cv::Scalar &color,
	                          const int waitTime = -1,
	                          const cv::Point offset = cv::Point(0, 0)) {
		if (srcImg.channels() == 1)
			cvtColor(srcImg, dstImg, cv::COLOR_GRAY2BGR);
...
template <typename RecType> struct showRectangle_wrapper<RecType, false> {
	static void showRectangle(const std::string &windowName,
...
void showRectangle(const std::string &windowName, const cv::Mat &srcImg,
                   cv::Mat &dstImg, RecType &rec, const cv::Scalar &color,
                   const int waitTime = -1,
                   const cv::Point offset = cv::Point(0, 0)) {
	using RecTypeWithoutRef = typename std::remove_reference<RecType>::type;
	showRectangle_wrapper<RecTypeWithoutRef>::showRectangle(
	    windowName, srcImg, dstImg, rec, color, waitTime, offset);
}

template <typename RecsContainer, bool = std::is_pointer<RecsContainer>::value>
struct showRecs_wrapper {};

template <typename Recs> struct showRecs_wrapper<Recs, true> {
	static void showRectangles(const std::string &windowName,
	                           const cv::Mat &srcImg, cv::Mat &dstImg,
	                           Recs &recs_or_recPtrs, const cv::Scalar &color,
	                           const int waitTime = -1,
	                           const cv::Point offset = cv::Point(0, 0)) {
		if (srcImg.channels() == 1)
			cvtColor(srcImg, dstImg, cv::COLOR_GRAY2BGR);
...
template <typename Recs> struct showRecs_wrapper<Recs, false> {
	static void showRectangles(const std::string &windowName,
	                           const cv::Mat &srcImg, cv::Mat &dstImg,
	                           Recs &recs_or_recPtrs, const cv::Scalar &color,
	                           const int waitTime = -1,
...
void showRectangles(const std::string &windowName, const cv::Mat &srcImg,
                    cv::Mat &dstImg, RecsContainer &recs_or_recPtrs,
                    const cv::Scalar &color, const int waitTime = -1,
                    const cv::Point offset = cv::Point(0, 0)) {
	using RecsWithoutRef = typename std::remove_reference<RecsContainer>::type;
	showRecs_wrapper<RecsWithoutRef>::showRectangles(
	    windowName, srcImg, dstImg, recs_or_recPtrs, color, waitTime, offset);
}
...
template <typename T>
void drawCrossing(const cv::Mat &srcImg, cv::Mat &dstImg,
                  const cv::Point_<T> &pt, const cv::Scalar &color) {
	if (srcImg.channels() == 1)
		cvtColor(srcImg, dstImg, cv::COLOR_GRAY2BGR);
	else if (srcImg.data != dstImg.data)
		srcImg.copyTo(dstImg);
	...
	cv::line(dstImg, Point_t(Vec_t(pt) + Vec_t(-10, 0)),
	         Point_t(Vec_t(pt) + Vec_t(10, 0)), color, 2);
	cv::line(dstImg, Point_t(Vec_t(pt) + Vec_t(0, -10)),
	         Point_t(Vec_t(pt) + Vec_t(0, 10)), color, 2);
}
